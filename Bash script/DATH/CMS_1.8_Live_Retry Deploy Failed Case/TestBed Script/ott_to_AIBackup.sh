...
OttCdnPath="/NAS_INGEST01/ott_rvod"
...
function move_ott_file_to_AIBackup {
	...
	do
		...
		if ! [ -f ${AssetInstaller_Backup}/${m3u8} ]
		then
			mv ${OttCdnPath}/${Asset_File} ${AssetInstaller_Backup}/
			print_log "mv ${OttCdnPath}/${Asset_File} ${AssetInstaller_Backup}/"
			mv ${OttCdnPath}/${m3u8} ${AssetInstaller_Backup}/
			print_log "mv ${OttCdnPath}/${m3u8} ${AssetInstaller_Backup}/"
		else
			if [ `stat -c %s $AssetInstaller_Backup/$m3u8` -eq 0 ];then
				...
				print_log "Already ${m3u8} in AssetInstaller_Backup"
			fi
		fi
	done
}
