...
numberOfGrep=`cat "${homeFolder}/sshScript/password" | awk '{print $1}' | grep ${host} | wc -l`
if [ ${numberOfGrep} -gt 1 ]
then
	resultArray=(`cat "${homeFolder}/sshScript/password" | awk '{print $1}' | grep ${host}`)
	...
	result=`cat "${homeFolder}/sshScript/password" | awk '{print $1}' | grep ${host}`
fi

combination=`grep -w ${result} "${homeFolder}/sshScript/password"`
...
# Start SSH Session
${homeFolder}/sshScript/sshScript.exp ${ip} ${user} ${password}
